# -*- coding: utf-8 -*-
"""
继续训练：30 epochs → 70 epochs
复用已有模型，继续训练40个新epochs
"""
...
def continue_training_30_to_70(model_path):
    """从30 epochs继续训练到70 epochs"""
    from ultralytics import YOLO
    
    print("\n📈 继续训练：30 epochs → 70 epochs")
    print("   预计时间：约50-60分钟（节省了30个epochs的训练时间）")
    
    model = YOLO(model_path)
    
    results = model.train(
        data="第4次实验数据及提交格式/data.yaml",
        epochs=70,           # 训练到70 epochs
        batch=8,
        imgsz=512,
        optimizer='AdamW',
        lr0=0.001,
    ...
    return "runs/detect/yolov8n_70epochs/weights/best.pt"
...
def main():
    print("="*70)
    print("CONTINUE TRAINING: 30 → 70 EPOCHS")
    print("复用已有模型，继续训练40个新epochs")
    print("="*70)
    ...
    # 2. 检查是否已有70 epochs模型
    model_70_path = "runs/detect/yolov8n_70epochs/weights/best.pt"
    
    if os.path.exists(model_70_path):
        print("\n✓ 找到已训练到70 epochs的模型！")
        model_path = model_70_path
    else:
        # 3. 继续训练40个epochs
        print("\n2. 继续训练40个新epochs（约50-60分钟）...")
        model_path = continue_training_30_to_70(model_path)
