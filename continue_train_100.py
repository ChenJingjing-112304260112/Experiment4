# -*- coding: utf-8 -*-
"""
继续训练：30 epochs → 100 epochs
复用已有模型，继续训练70个新epochs
"""
...
def continue_training_30_to_100(model_path):
    """从30 epochs继续训练到100 epochs"""
    from ultralytics import YOLO
    
    print("\n📈 继续训练：30 epochs → 100 epochs")
    print("   预计时间：约80-90分钟（节省了30个epochs的训练时间）")
    
    model = YOLO(model_path)
    
    results = model.train(
        data="第4次实验数据及提交格式/data.yaml",
        epochs=100,          # 训练到100 epochs
        batch=8,
        imgsz=512,
        optimizer='AdamW',
        lr0=0.001,
    ...
    return "runs/detect/yolov8n_100epochs/weights/best.pt"
...
def main():
    print("="*70)
    print("CONTINUE TRAINING: 30 → 100 EPOCHS")
    print("复用已有模型，继续训练70个新epochs")
    print("="*70)
    ...
    # 2. 检查是否已有100 epochs模型
    model_100_path = "runs/detect/yolov8n_100epochs/weights/best.pt"
    
    if os.path.exists(model_100_path):
        print("\n✓ 找到已训练到100 epochs的模型！")
        model_path = model_100_path
    else:
        # 3. 继续训练70个epochs
        print("\n2. 继续训练70个新epochs（约80-90分钟）...")
        model_path = continue_training_30_to_100(model_path)
